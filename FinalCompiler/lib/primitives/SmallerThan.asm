...
LsmallerThanLoop:
	...
	JUMP_EQ(LsmallerThanSetInteger);
  LsmallerThanSetFraction:
	...
	JUMP(LsmallerThanLoopCont);
  LsmallerThanSetInteger:
  ...
  LsmallerThanLoopCont:

	DECR(R2); // i--
	INCR(R3); // next arg

	CMP(R2,IMM(0));
	JUMP_EQ(LSmallerThenloopEnd);

	CMP(IND(FPARG(R3)),IMM(T_FRACTION));
	JUMP_EQ(LsmallerThanLoopSetFraction);

   LsmallerThanLoopSetInteger:
	//SHOW("INT - MULTIPLYING R5 WITH:",INDD(FPARG(R3),1));
	MUL(R5,INDD(FPARG(R3),1));
	JUMP(LsmallerThanLoopContinue);
   LsmallerThanLoopSetFraction:
	...
	JUMP(LsmallerThanLoopContinue);
   LsmallerThanLoopContinue:
	CMP(R4,R5);
	//SHOW("LEFT AFTER MULT IS:",R4);
	//SHOW("RIGHT AFTER MULT IS:",R5);
	JUMP_GE(LsmallerThanLoopEndFalse);

	/* DECR(R2); // i--
	INCR(R3); // next arg */
	
	JUMP(LsmallerThanLoop);
...
LSmallerThenloopEnd:
	...
	//DROP(1);
	CMP(R1,IMM(1));
	JUMP_EQ(LsmallerThanLoopEndTrue);
	CMP(R1,IMM(0));
	JUMP_EQ(LsmallerThanLoopEndFalse);
	//JUMP(LsmallerThanExit);
	

LsmallerThanLoopEndTrue:
